...
// URL de la fiche Play Store (ajout) — un lien PWA classique
// (BETA_APP_URL, https://yoria.run) ouvert depuis une app tierce (Gmail,
// WhatsApp...) s'ouvre dans la WebView intégrée de cette app, où
// beforeinstallprompt ne se déclenche JAMAIS (limitation universelle des
// WebViews embarquées, pas un bug Yoria — confirmé en testant en
// conditions réelles depuis l'app Gmail). Le Play Store, lui, gère sa
// propre installation nativement et fonctionne de façon fiable peu importe
// le navigateur/WebView d'où vient le clic — c'est pourquoi un candidat
// Android reçoit ce lien plutôt que l'URL web classique. Codé en dur
// plutôt qu'en variable d'environnement : ce n'est pas un secret, et
// contrairement à BETA_APP_URL (qui peut légitimement changer de domaine),
// l'id de package Android est stable une fois publié.
...
// Instructions d'installation Android (mises à jour) — le lien pointe
// maintenant directement vers le Play Store (cf. PLAY_STORE_URL
// ci-dessus), donc les instructions se limitent à confirmer que le bouton
// mène bien à l'installation — plus besoin d'expliquer un mécanisme
// PWA/beforeinstallprompt qui ne se déclenchait pas de façon fiable
// depuis les apps email.
          ...
          <tr>
            <td style="padding:22px 24px;">
              ...
              <p style="
                ...
                line-height:1.6;
              ">
                Le bouton ci-dessus vous amène directement sur le Play
                Store — appuyez sur <strong>Installer</strong>, comme pour
...
// Instructions d'installation iOS — inchangées. Pas d'équivalent Play
// Store côté Apple (aucun pont officiel PWA-vers-App-Store, cf. inventaire
// §13), le lien pour un candidat iPhone reste donc l'URL web classique et
// ces instructions Safari restent la bonne — et seule — approche.
