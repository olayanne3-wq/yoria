# Notes pratiques par type de séance — Yoria v2.0

Document de référence méthodologique pour le mécanisme de notes pratiques (conseils d'exécution ciblés par famille de séance), implémenté dans `public/v2/engine/plan-generator.js`.

---

## 1. Principe

Une note pratique est un conseil court, ciblé sur **comment exécuter** une séance donnée — à distinguer des jalons narratifs (qui signalent un moment charnière du *plan*, pas un conseil sur la séance elle-même) et des repères qualitatifs (qui portent sur le *ressenti* et la *progression*, pas l'exécution).

**Origine** : la v1 avait ce type de conseil ponctuel sur certains types de séance (`"Hydratation++ · Allonge selon la forme"` sur une sortie longue). Le moteur v2 n'avait aucune note de ce type.
...
Ce mapping (`FAMILLE_SOUS_TYPE`) est **partagé** avec le mécanisme de repères qualitatifs — une seule source de vérité pour la notion de "famille de séance qualité", pas dupliquée entre les deux mécanismes.

## 3. Format d'intégration

Identique au principe des jalons narratifs : fusionné dans le champ `contenu` existant, par simple concaténation. `injecterNotesPratiques()` est indépendante de `injecterJalonsTransition()` — les deux peuvent s'appliquer à la même séance, auquel cas les deux notes se cumulent dans le contenu (jalon d'abord si les deux fonctions sont appelées dans cet ordre, mais l'ordre entre les deux n'a pas d'incidence fonctionnelle, seulement sur l'ordre d'affichage final).
...
**Absence volontaire d'une famille "test"** : la séance test (`sousType: 'test'`) n'a pas de note pratique dans ce mécanisme — cohérent avec le fait qu'elle a son propre traitement narratif complet dans le mécanisme de cohérence de la semaine test, qui couvre déjà largement ce moment.
...
- `public/v2/engine/plan-generator.js` — `FAMILLE_SOUS_TYPE` (mapping partagé avec les repères qualitatifs), `NOTES_PRATIQUES` (banque), `injecterNotesPratiques()` (détection + injection)
