...
4. Si plusieurs plans existent dans le Gist, `index-v2-preview.html` doit permettre d'en choisir un (pas juste charger "le dernier" par défaut) — probablement un sélecteur simple, réutilisant la même liste que celle affichée dans le wizard v2 (`afficherPlansSauvegardes`)
