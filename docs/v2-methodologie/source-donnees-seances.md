# Source de données des séances (Strava / manuel) — Yoria

Document de conception du chantier permettant à un utilisateur de suivre son plan et d'alimenter l'estimation 10K **sans dépendre de Strava** — posé avant la mise en ligne sur le Play Store, où les utilisateurs n'auront pas tous un compte Strava.
...
| `fit` | Disponible | Import d'un fichier `.fit` exporté depuis la montre (cf. inventaire §10) |
...
Les laps Strava sont détaillés par répétition (`getLapsAffichage`). Reproduire ce niveau de détail EN SAISIE (une allure par répétition) resterait trop lourd pour un usage sans montre connectée — ce principe reste inchangé. La **représentation stockée** : chaque répétition attendue de `structureIntervalles` produit son propre lap synthétique, à partir de la même allure globale saisie une seule fois, combinée à une grille réussi/raté (✓/✕) par répétition. Cf. inventaire §4/§5/§7bis pour le détail complet de l'implémentation.
...
- **Réussite par répétition** (✓/✕) — sert aussi à construire les laps individuels ci-dessus, en plus de son rôle d'origine (affichage du taux de réussite)

### Archive — format d'origine
...
Distance d'effort déductible de `structureIntervalles` si non précisée par l'utilisateur. Ce format reste le repli utilisé par `weightedAvgByEffortDuration()` pour toute saisie manuelle antérieure au champ `laps` (pas de migration rétroactive, cf. ci-dessus).
...
Persisté et synchronisé via Supabase au même titre que `statuses` (`LkSync`), via un merge atomique par RPC (cf. inventaire §5) qui élimine les races conditions d'écriture concurrente.

---

## 6. Points tranchés

Les points historiquement ouverts sur ce chantier sont désormais réglés :

- `manualPerf` suit le même chemin de synchronisation Supabase que `statuses` — confirmé, synchronisé exactement comme toute autre clé préfixée par plan via `synchroniserVersSupabase()`/`plan_donnees`.
- Le bloc "Réalisé" affiche la source de la donnée retenue (badge Strava/FIT/manuel, cf. inventaire §4) — suffisant pour la transparence recherchée, pas de badge dédié supplémentaire jugé nécessaire.
- Les options `montre` et `gpx` restent volontairement non fonctionnelles à ce jour, `fit` étant la seule à être passée de "non implémentée" à disponible (cf. tableau §3 ci-dessus, cf. inventaire §10).
