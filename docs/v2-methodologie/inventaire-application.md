...
| Chantier | Statut |
|---|---|
| Système de badges (récompenses) | 🔜 Conception discutée en profondeur le 27/07/2026 avec Laurent (pas codée). **Principe directeur acté** : récompenser ce que Yoria valorise déjà (régularité, respect du corps, progression durable), jamais la performance brute ou l'intensité — pour ne pas pousser vers un comportement à risque (surentraînement pour débloquer un badge). **4 catégories actées**, chacune avec sa couleur dérivée de la charte existante (jamais de nouvelle couleur "gamification" façon or/étoile) : Régularité (turquoise `--accent2`) ; Progression (bleu `--accent`) ; Respect du corps (vert sauge, nouvelle nuance introduite spécifiquement) ; Étapes du plan (gris neutre, jalons automatiques). **Explicitement écarté** : badges de volume/distance brute et d'intensité/vitesse (risque de pousser à en faire trop) ; tout classement ou comparaison sociale. **Identité visuelle actée** : anneau de progression fin (pas de médaille/étoile), qui se remplit visuellement pour les badges à paliers (affichage type "6/10" au centre) plutôt que binaire débloqué/verrouillé. Icônes en traits fins linéaires (pas d'emoji). Badge verrouillé : anneau en pointillés, icône à opacité réduite (0.35) — JAMAIS masqué de la liste (voir tous les badges obtenables donne un objectif clair, contrairement à une mécanique de surprise type loot-box). **Emplacement acté** : rien en permanence sur le dashboard principal — seul un bandeau de notification ponctuel et dismissible au moment d'un déblocage (même famille visuelle que `adaptationEl`/`moteurDecisionEl`). Écran détaillé accessible via une carte cliquable tout en haut de l'écran Stats (pas Paramètres — Stats est déjà l'endroit où la progression se consulte).

**Règles de calcul actées le 27/07/2026 (toujours pas codées, prêtes pour l'implémentation) :**

*Badges à paliers (anneau progressif, RECORD HISTORIQUE jamais perdu si la série casse — décision explicite pour éviter l'effet "streak" anxiogène façon Duolingo, qui pousserait à forcer une séance plutôt que d'écouter une vraie pause légitime, cf. `reduire_charge`/`alerter_blessure_potentielle`) :*
- 🔥 Séances validées d'affilée (5/10/20/40) — plus longue série historique de `statutEffectif ∈ {✅, ⚠️}` consécutifs sur `ALL_SESSIONS` triées par date (❌ ou absence de statut sur une séance passée casse la série). ⚠️ compte comme une réussite pour la série, cohérent avec `calculerScoreSemaine`/`weeklyReport` qui traitent déjà ⚠️ comme "en partie réussi", pas un échec — éviter une règle divergente entre badges et reste de l'app.
- 📅 Semaines complètes d'affilée (2/4/8/16) — `weeklyReport(weekNum).done + adapted === total`, série max sur l'historique des semaines passées.
- 🎯 FC EF/LONGUE maîtrisée d'affilée (2/4/8) — granularité SEMAINE (pas séance individuelle, décision actée : `avgEfHr` déjà existant est une moyenne hebdo, une granularité séance nécessiterait de rouvrir la fragilité connue de `getLapsAffichage`/`getEffortLaps` sur les séances EF sans vraie structure de laps ; graularité différente du signal coach ajouté le même jour aurait aussi été incohérente) — série de semaines où `avgEfHr` reste dans `zoneFC.zonesParType.E`.

*Badges événementiels (un seul niveau, débloqué/verrouillé, pas de paliers) :*
- 📈 Nouvelle estimation confirmée — `calculerReferenceCouranteAllures().statut === 'progression'` (déjà calculé toutes les 2 semaines paires, cf. §7 allures dynamiques).
- 🏅 Record personnel battu — nouveau temps saisi bat `profilCoureur.records` sur une distance.
- 🧘 Repos écouté — décision `reduire_charge` ACCEPTÉE (bouton Appliquer cliqué dans `DecisionEngineApply`), pas juste proposée.
- ⚖️ Semaine équilibrée — `risque === 'faible'` (ratio ACWR 0.8-1.3) sur 4 semaines d'affilée.
- 🚀 Premier plan lancé — création du tout premier plan du compte.
- 🏔️ Mi-parcours franchi — semaine courante atteint la moitié de `dureeSemaines`.
- 🎽 Entrée en Affûtage — `phaseAtDate()` (déjà codée le 27/07/2026, cf. §7) passe à `'Affutage'` pour la première fois sur ce plan.
- 🏁 Course terminée — date de course dépassée, plan encore actif au moment du chargement.
- 🔁 Retour réussi — une semaine ayant déclenché `analyserAdaptations` (semaine "à adapter", cf. §16 garde-fou d'exclusion) suivie d'une semaine complète réussie (`weeklyReport` : `done+adapted===total`) — réutilise deux mécanismes déjà en place plutôt qu'un nouveau seuil en nombre de séances.

**Reste à concevoir avant codage** : mécanisme de stockage (Supabase, probablement une nouvelle table `badges_debloques` ou un champ JSON sur le profil coureur — jamais recalculé à chaque `render()` pour les badges événementiels historiques, seulement pour détecter un NOUVEAU déblocage), détection du moment de déblocage (calcul en arrière-plan périodique vs événement dédié à chaque écriture de statut/décision), et si les paliers atteints doivent aussi déclencher une notification à chaque niveau ou seulement au premier déblocage de la catégorie |
