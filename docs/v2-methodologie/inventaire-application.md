...
| Permettre de changer la date de course d'un plan actif | 🔜 Identifié le 27/07/2026 : aucun mécanisme n'existe aujourd'hui pour modifier `dateCourse` sur un plan déjà généré, ni dashboard ni wizard — l'accordéon "Modifier mon plan" (`public/v2/index.html`, cf. §3) ne couvre que 3 leviers (Objectif/Jours/Volume), qui laissent tous `dateCourse` intact. Différent en nature des 3 leviers existants : changer la date modifie le nombre de semaines RESTANTES, donc touche potentiellement toute la structure de phases — pas un simple ajustement dans une taille de plan fixe. Conception discutée en profondeur le 27/07/2026 (pas codée), mécanisme et règles de phase actés : 1) **Régénération continue, pas un ré-appel `generatePlan()` complet** — la semaine en cours et toutes les semaines passées ne sont JAMAIS régénérées ; `computeVolumeProgression()` doit évoluer pour accepter un point de reprise (numéro de semaine réel `s`, volume réellement atteint) au lieu de toujours repartir de `s=1` et du `volumeDepart` initial du plan — un simple ré-appel complet façon `completerPlanApresTestSemiCooper()` désynchroniserait le cycle de décharge (`estDecharge = s % 4 === 0`, calé sur `s` compté depuis la régénération, pas depuis le vrai début) et ferait repartir le volume de trop bas. 2) **Décalage ≥ 8-9 semaines** : recommander de créer un nouveau plan plutôt que de prolonger l'existant, sans bloquer — l'utilisateur reste libre de prolonger quand même. 3) **Phase Spécifique en cours** : jamais de retour en Construction quel que soit le recalcul brut — la suite reste en Spécifique (prolongée) ou avance vers Affûtage selon le temps réellement disponible. 4) **Phase Affûtage en cours, décalage ≤ 1 semaine** : reste en Affûtage tel quel, avertissement informatif seulement (pas de recalcul de phase, cohérent avec le pattern `warnings` déjà en place dans le moteur — jamais de blocage silencieux). 5) **Phase Affûtage en cours, décalage > 1 semaine** : repasse en Spécifique pour la durée nécessaire, afin qu'un vrai Affûtage de durée normale (`DUREE_AFFUTAGE_JOURS`) se recalcule juste avant la nouvelle date de course — règle qui évite tout Affûtage anormalement étiré ou tout retour en arrière physiologique incohérent (l'Affûtage existe pour arriver frais, pas pour remonter en charge). 6) **Anticipation** : si le changement est connu et appliqué pendant qu'on est encore en Spécifique (avant d'entrer en Affûtage), prolonger directement la Spécifique en cours plutôt que d'attendre d'être en Affûtage pour devoir en ressortir — même règle que le point 3, observée à un moment différent, pas un mécanisme séparé. Reste à concevoir avant codage : formulation exacte des avertissements (Affûtage court, décalage massif), détection précise de chaque cas, et signature exacte du point de reprise à ajouter à `computeVolumeProgression()`/`computePhases()` |
