...
| Chantier | Statut |
|---|---|
| Système de badges (récompenses) | 🔜 Conception discutée en profondeur le 27/07/2026 avec Laurent (pas codée). **Principe directeur acté** : récompenser ce que Yoria valorise déjà (régularité, respect du corps, progression durable), jamais la performance brute ou l'intensité — pour ne pas pousser vers un comportement à risque (surentraînement pour débloquer un badge). **4 catégories actées**, chacune avec sa couleur dérivée de la charte existante (jamais de nouvelle couleur "gamification" façon or/étoile) : Régularité (turquoise `--accent2`, ex. séries de séances validées, semaines complètes, retour réussi après une pause) ; Progression (bleu `--accent`, ex. nouvelle estimation confirmée via `calculerReferenceCouranteAllures`, record personnel battu) ; Respect du corps (vert sauge, nouvelle nuance introduite spécifiquement — ex. décision `reduire_charge` acceptée plutôt qu'ignorée, respect répété de la zone FC EF cf. §8) ; Étapes du plan (gris neutre, jalons automatiques du moteur — premier plan, mi-parcours, entrée en Affûtage, course terminée). **Explicitement écarté** : badges de volume/distance brute et d'intensité/vitesse (risque de pousser à en faire trop, contraire à la philosophie de progression prudente déjà ancrée dans le moteur) ; tout classement ou comparaison sociale (Yoria reste un outil solo). **Identité visuelle actée** : anneau de progression fin (pas de médaille/étoile), qui se remplit visuellement pour les badges à paliers (ex. "6/10" affiché au centre) plutôt que binaire débloqué/verrouillé — élément signature du système. Icônes en traits fins linéaires (pas d'emoji), un pictogramme par badge. Badge verrouillé : anneau en pointillés, icône à opacité réduite (0.35) — jamais totalement masqué de la liste, principe explicitement acté avec Laurent ("voir tous les badges obtenables, y compris ceux pas encore débloqués donne un objectif clair, contrairement à une mécanique de surprise type loot-box"). **Emplacement acté** : PAS sur le dashboard principal en permanence (prend de la place pour une info qui ne change pas au quotidien) — seul un bandeau de notification ponctuel et dismissible apparaît au moment d'un déblocage (même famille visuelle que `adaptationEl`/`moteurDecisionEl`, bordure gauche colorée). Écran détaillé (tous les badges, groupés par catégorie, barre de progression globale en tête) accessible via une carte cliquable tout en haut de l'écran Stats — jugé plus logique qu'une section Paramètres (Stats est déjà l'endroit où la progression se consulte : ACWR, monotonie, records). **Reste à concevoir avant codage** : règles de calcul précises de chaque badge (seuils exacts, quelles données du moteur les alimentent — `statutEffectif`, `analyserAdaptations`, `weeklyReport`, `predHistory`...), mécanisme de stockage (Supabase, probablement une nouvelle table ou un champ sur le profil), détection du moment de déblocage (calcul à chaque `render()` vs événement dédié) |
