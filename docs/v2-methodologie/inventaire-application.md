...
- **Avant TOUT push d'un fichier vers le repo, vérifier explicitement le
  ...
  hors des balises `<script>`. Incident réel le 07/08/2026 : ce texte
  parasite affiché littéralement en haut de la page en production, non
  détecté avant que Laurent ne le signale par capture d'écran.

## 15bis. Écrans statiques hors JS (splash, chargement)

**Écran de chargement (logo Y, entre le splash Android natif et le
premier `render()`, 07/08/2026)** — HTML/CSS pur dans le `<body>`
d'`index.html`, visible dès le tout premier paint, avant tout script.
Dimensions calculées en JavaScript synchrone (pixels absolus,
`window.innerWidth`) plutôt qu'en CSS pur — `width:Xvw`/`aspect-ratio`
se sont révélés peu fiables sur la vraie WebView Android (confirmé
différent du rendu Chromium desktop/Playwright, qui les validait à
tort comme corrects). Toute calibration future de ce type d'écran
(position, taille, couleur) doit être mesurée sur de vraies vidéos de
contrôle de l'appareil cible, jamais uniquement sur un rendu desktop.
**Repères visuels temporaires (bordure + ligne à la position cible,
couleur n'existant nulle part ailleurs dans l'app) se sont révélés
décisifs** pour lever plusieurs ambiguïtés de mesure qui avaient fait
échouer plusieurs itérations de correctifs à l'aveugle — à retirer une
fois la calibration validée, jamais laissés en production.
...
| Clignotement de l'écran au chargement de l'app | ✅ Résolu le 07/08/2026, après plusieurs itérations menées le 04/08/2026 (partielles) puis reprises le 07/08/2026 avec une méthode plus rigoureuse. Cause racine finalement identifiée : plusieurs sources indépendantes de re-render automatique juste après le premier affichage (badges débloqués, météo actuelle, météo J+1 — cf. §4/§11), dont les latences réseau respectives dépassaient la fenêtre de regroupement de `renderDiffere()` (50ms), chacune produisant son propre flash visible. Corrigé par un compteur explicite des 3 sources (`renderDiffereChargementInitial()`, fenêtre dédiée qui n'attend PAS un délai fixe mais la résolution effective des 3 sources, avec filet de sécurité à 15s) — remplace l'approche par délai fixe (150ms puis 600ms, toutes deux insuffisantes ou trop lentes). Un second problème distinct (fondu CSS d'entrée relancé sur un contenu déjà stable par ces re-renders) a été corrigé en désactivant l'animation `fadeIn` sur toute la fenêtre de chargement initial, pas seulement le tout premier `render()`. Diagnostic final confirmé par analyse frame-par-frame de plusieurs vidéos de contrôle (voir la ligne ci-dessous, écran de chargement, pour la suite de ce chantier). |
| **Écran de chargement avec logo (nouveau, 07/08/2026)** | ✅ Codé et poussé. Remplace l'écran blanc entre le splash Android natif et le premier rendu du dashboard par un écran statique HTML/CSS pur (aucune dépendance JS), visible dès le tout premier paint du navigateur : logo Y sur fond dégradé marine, reproduisant fidèlement l'icône `icon-512.png` (couleurs, taille, position mesurées directement sur des vidéos de contrôle du splash natif — plusieurs itérations nécessaires, cf. historique détaillé en commentaire dans `index.html`). Dimensions calculées en JavaScript synchrone (pixels absolus) plutôt qu'en CSS pur (`vw`/`aspect-ratio` se sont révélés peu fiables sur la WebView Android réelle, contrairement au rendu desktop Playwright qui les validait à tort). Slogan "Ton coach running personnel" ajouté sous le logo (repris du `manifest.json`), Inter italique léger. Masqué par `render()` dès le tout premier rendu réel. Méthode de diagnostic notable : des repères visuels temporaires (bordure + ligne magenta à la position cible) ont permis de lever plusieurs ambiguïtés de mesure, retirés une fois la calibration validée — cf. §15 pour le principe général. |
...
| **Fix texte parasite en tête de fichier (07/08/2026)** | ✅ Corrigé. Un message de statut d'un outil de lecture (`"successfully downloaded text file..."`), concaténé par erreur en tête d'`index.html` lors d'une extraction/reconstruction de fichier, s'affichait littéralement en haut de la page en production. Retiré. Nouvelle consigne de vérification systématique (début/fin de fichier) avant tout push, pour ne pas reproduire cette erreur. |
| **Fix grille de la carte semaine ne reflétant pas les séances décalées/swap (07/08/2026)** | ✅ Corrigé. `renderGrilleJoursSemaine()` (grille des 7 jours dans chaque carte de semaine, onglet Semaines) lisait `week.sessions[i]` directement, sans passer par `getEffectiveSession()` (déjà utilisée partout ailleurs pour tenir compte de `swappedSessions`) — une séance déplacée continuait d'afficher son type d'origine sur cette grille, alors que le détail de semaine affichait bien le nouveau type après échange. Un second point du même type a été trouvé et corrigé dans le graphique FC par type de séance (Stats) : l'axe des semaines était calculé depuis `week.sessions` brut alors que les points affichés sur ce même graphique utilisent déjà `getEffectiveSession` via `ALL_SESSIONS`. |
