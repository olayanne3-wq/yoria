...
- Jamais d'apostrophe dans une chaîne JS entre guillemets doubles (échec silencieux du parseur). 404 sur une route API → vérifier `vercel.json` en premier (toute nouvelle route `api/*.js` doit y être déclarée explicitement, sinon la requête tombe sur le catch-all statique et échoue silencieusement — sans lien avec le code du endpoint lui-même).
...
- Une fonction générique conçue pour un usage (déterminer elle-même un paramètre à partir d'une position calculée) peut se révéler mal adaptée à un second usage qui a besoin de FORCER ce même paramètre explicitement — si un même paramètre pilote à la fois "quoi choisir" et "combien en proposer" (ex. position dans une rotation qui détermine à la fois le sous-type ET le nombre de répétitions), le contourner pour forcer l'un casse silencieusement l'autre. Un outil de forçage explicite doit alors sortir complètement de la fonction générique plutôt que de la contourner par un paramètre artificiel (cf. l'outil "Corriger une séance qualité", passé en saisie manuelle directe après un premier essai qui produisait un nombre de répétitions incohérent, `site-beta-admin.md`).
- Un mécanisme d'affichage qui résout une position "vue" vers une position "physique" (ex. `swapTable`/`getEffectiveSession()` pour les séances échangées, cf. `architecture-generale.md`) doit être documenté explicitement dans toute fonction PONT consommée en dehors du contexte qui connaît cette résolution (ex. `v1-bridge.js`, utilisée aussi côté serveur par `beta-admin`) — sans quoi une position communiquée par un humain ("la séance de vendredi") peut ne correspondre à aucune donnée réelle si cette séance a été déplacée depuis sa position d'origine.
...
- Une action rare (créer un nouveau plan, changer de plan actif) ne justifie pas une place fixe sur l'écran le plus consulté de l'app (le dashboard) — la déplacer vers un écran thématiquement cohérent avec l'action, même si moins fréquenté au quotidien (cf. le sélecteur de plan, déplacé du dashboard vers l'onglet Course).
...
- `swapTable` n'affecte QUE l'affichage côté client — `assignment` reste
  toujours la structure physique d'origine. Tout code externe qui
  consomme un plan (via `traduirePlanVersFormatV1()`, ex. `beta-admin`)
  obtient systématiquement la position physique, jamais résolue —
  détail complet dans `architecture-generale.md`.
...
- Déplacés du dashboard vers l'onglet Course (20/08/2026) : action rare
  (changer de plan actif, en créer un nouveau), ne justifiait plus une
  place fixe sur l'écran le plus consulté de l'app. Onglet Course
  désormais toujours visible dans la nav, y compris en Mode Forme —
  affiche un état vide (sélecteur + bouton "+ Nouveau plan") si aucun
  plan course actif. Détail complet dans `architecture-generale.md`.
