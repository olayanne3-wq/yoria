...
| Chantier | Statut |
|---|---|
| Revoir `repartirVolumeSemaine()` pour les profils à peu de jours disponibles (0 EF) | 🔜 Diagnostiqué et conception discutée en profondeur avec Laurent le 27/07/2026, PAS CODÉ. **Bug identifié** : `RATIO_LONGUE = 0.28` (fixe, `plan-generator.js`) suppose implicitement plusieurs séances EF en plus de la longue — avec seulement 2 jours disponibles (qualité + longue, `nbEF=0`), la longue ne récupère que 28% du volume total peu importe le volume choisi, obligeant `VOLUME_JOURS_INCOMPATIBLE` (garde-fou existant, `VOLUME_MIN_LONGUE_KM=5`) à forcer un volume de départ artificiellement élevé (ex. 17km) pour qu'un simple calcul (`17×0.28=4.76`, sous le seuil) passe de justesse — sans lien avec la vraie pratique de l'utilisateur. Symptôme observé par Laurent : dashboard affichant "17km prévus" alors que le contenu réel des séances générées ne totalisait que ~8km (écart de 9km entre volume cible et volume réellement casé dans les séances). **Conception actée** : ratio de la longue devient progressif selon `nbEF` (pas un simple `if nbEF===0`) — proche de 28% quand `nbEF` est élevé (comportement actuel inchangé), progressivement plus généreux à mesure que `nbEF` diminue vers 0. Simulations faites avec Laurent : un ratio fixe à 50% du volume total (pour `nbEF=0`) suffit à passer le seuil de 5km dès 10km/semaine (contre 17km minimum avant), mais laisse un écart résiduel affiché/réel (~2km non utilisés) — jugé acceptable comme compromis. **Contrainte ferme actée, priorité absolue** : la sortie longue doit TOUJOURS rester la séance la plus longue de la semaine, quel que soit le sous-type de qualité généré (certains sous-types comme `seuil-negatif`/`tempo-court` peuvent faire 9-12km, dépassant une longue calculée uniquement par ratio — vérifié par simulation : avec `nbEF=0` et qualité=seuil-negatif à 12km, même absorber 100% du `kmRestant` ne suffit pas à garantir longue>qualité). Solution retenue : `kmLongue = max(ratio_calculé, kmQualiteTotal + marge)` avec une marge modeste (+1 à +2km), quitte à dépasser légèrement le volume cible total dans les cas extrêmes — jugé préférable à une incohérence de rôle (une "sortie longue" plus courte qu'une séance qualité contredit sa propre définition, pas juste une question d'esthétique de répartition). **Reste à faire avant codage** : définir la formule exacte de progressivité du ratio selon `nbEF` (pas encore choisie précisément — seul le cas `nbEF=0` a été simulé), et la valeur exacte de la marge au-dessus de `kmQualiteTotal` |
