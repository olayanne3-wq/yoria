...
| Permettre de changer la date de course d'un plan actif | 🔜 Identifié le 27/07/2026 : aucun mécanisme n'existe aujourd'hui pour modifier `dateCourse` sur un plan déjà généré, ni dashboard ni wizard — l'accordéon "Modifier mon plan" (`public/v2/index.html`, cf. §3) ne couvre que 3 leviers (Objectif/Jours/Volume), qui laissent tous `dateCourse` intact. Différent en nature des 3 leviers existants : changer la date modifie le nombre de semaines RESTANTES, donc `computePhases()` recalculerait `totalSemaines`/`constructionSemaines`/`specifiqueSemaines` en profondeur (pas un simple ajustement dans une structure de taille fixe) — plus proche en complexité de `completerPlanApresTestSemiCooper()` (qui ré-appelle `generatePlan()` avec une date décalée, semaines déjà vécues jamais régénérées) que des leviers légers actuels. Conception discutée le 27/07/2026 (pas codée) : mécanisme retenu = ré-génération complète du reste du plan via `generatePlan()` à partir de la semaine suivante (même principe que le test semi-Cooper), jamais de distinction arbitraire "petit vs gros décalage" par seuil de jours. Cas problématique identifié et tranché : si la semaine en cours est déjà en phase Affûtage au moment du changement, un recalcul naïf peut soit faire "redescendre" le plan en Construction/Specifique (recul de date = plus de temps disponible — incohérent physiologiquement, l'Affûtage existe pour arriver frais, pas pour remonter en charge après), soit étirer l'Affûtage sur une durée anormalement longue (également incohérent — sa durée est volontairement fixe et courte, `DUREE_AFFUTAGE_JOURS`). Décision actée : ni blocage dur, ni recalcul silencieux — avertissement explicite systématique si le changement impacte une semaine déjà en Affûtage (retour en arrière détecté OU Affûtage significativement rallongé), cohérent avec le pattern `warnings` déjà en place partout ailleurs dans le moteur (jamais de blocage silencieux, responsabilité finale laissée à l'utilisateur qui connaît le contexte réel). Reste à concevoir avant codage : formulation exacte de l'avertissement, détection précise du cas "impact sur l'Affûtage" (comparer phase de la semaine en cours avant/après recalcul), et si un blocage dur reste justifié pour le cas le plus extrême (retour Affûtage→Construction) malgré la préférence pour l'avertissement seul |
