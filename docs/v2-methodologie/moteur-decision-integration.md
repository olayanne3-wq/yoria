...
| Code du moteur à structurer depuis zéro | Un dossier `/engine-classic-scripts/` existe déjà, avec des modules séparés chargés en `<script src>` — le moteur peut suivre exactement ce pattern. **Note du 19/07/2026 (session ultérieure)** : les exemples cités à l'origine (`plan-generator.classic.js`, `plan-forme.classic.js`, `weather.classic.js`) ont depuis été supprimés — ces modules-là sont maintenant chargés directement depuis `public/v2/engine/*.js` via `import()` dynamique (chantier de conversion en modules ES, cf. `convergence-v1-v2.md` §12), éliminant leur duplication. Les 8 `decision-engine-*.classic.js` (dont il est question dans ce document) n'ont, eux, jamais eu de version module ES source — ils restent, à raison, dans `/engine-classic-scripts/` en scripts classiques, chargés en `<script src>` exactement comme décrit ici. |
