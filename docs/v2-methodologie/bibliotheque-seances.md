...
## 43. Manque identifié — pas de plafond de volume par séance pour les zones I/T/V/C

Signalé par Laurent le 06/08/2026 : une séance `4×3min @ I, récup 2min` (12 min cumulées de VMA) lui paraissait trop difficile, malgré une allure I correctement calibrée sur sa forme mesurée (4:21/km, confirmée par 5 séances récentes) et un nombre de répétitions cohérent avec le `base` niveau intermédiaire du moteur (`i-3min`, section 12).

**Diagnostic** : le moteur limite bien le nombre de répétitions par sous-type (`base`/`cap` par niveau, section 12), mais ne borne nulle part le **volume total d'une séance qualité individuelle** — seule la durée par répétition est cadrée par la fourchette 3-5 min pour I (section 1). Élargi lors de la discussion du 06/08 : le même manque existe pour T et V, chacune ayant son propre plafond dans Daniels (voir tableau ci-dessous) — pas seulement I.

**Règle officielle retrouvée dans le livre Daniels (chapitre 4, figure 4.1, fichier projet)** : le volume par **séance qualité individuelle** (pas le cumul de toutes les séances qualité de la semaine — texte exact : "the percentage of weekly miles that is typically associated with **a single session**") est plafonné, avec un pourcentage propre à chaque zone :
...
| C (Allure course) | `allure-course`, `allure-course-court`, `test` (confirmation d'allure, section 36) | min(20 min×2 = 18 miles/29km, **20%** volume hebdo) — repère "M" chez Daniels, seule zone dont Daniels chiffre un plafond apparenté à C dans ce document |

Si la semaine a 2 séances qualité (ex. une I et une T, ou deux fois la même zone), **chacune est plafonnée indépendamment** à son propre pourcentage du volume hebdo — pas de partage ni de division du budget entre les deux séances de la semaine. Ce plafond par séance est distinct de la règle 80/20 (section 1), qui elle porte sur le cumul hebdomadaire toutes qualités confondues.

**Cas de la séance test (section 36) — vérifié le 06/08/2026** : la séance test/confirmation d'allure utilise l'allure C, donc soumise au même plafond M/Daniels (20% du volume hebdo). Pour le cas concret de Laurent (10K, distance test 5,5km, volume hebdo 33km/sem), la séance reste sous le plafond (6,6km max) — pas de problème actuellement. Mais les pourcentages de distance de la section 36 (60% pour 5K, 55% pour 10K, 35% pour Semi, 25% pour Marathon) n'avaient jamais été vérifiés contre ce plafond au moment de leur conception. Calcul du seuil de volume hebdo sous lequel chaque distance dépasse le plafond :

| Distance | Distance test générée | Dépasse le plafond M si volume hebdo < |
|---|---|---|
| 5K | 3,0 km | 15 km |
| 10K | 5,5 km | 27,5 km |
| Semi | 7,4 km | 37 km |
| Marathon | 10,6 km | 53 km |

Comparé aux plafonds de population débutant (section 2 : 10K 25-35km, Marathon 35-40km), un **débutant en tout début de progression** (avant d'atteindre le pic de volume de sa phase) peut se retrouver avec une séance test qui dépasse ce plafond — trou distinct de celui trouvé sur I/T/V, à couvrir dans la même implémentation puisqu'il s'agit du même mécanisme de plafonnement.

**Cas concret vérifié** : Laurent, volume hebdo 33km/sem, allure I 4:21/km → plafond 8% = 2,64km ≈ **11,5 min cumulées**. La séance générée (12 min) dépasse ce plafond d'environ 30 secondes — léger dépassement, pas extrême, mais réel et confirmé, pas une impression.

**Pas encore implémenté** : aucun garde-fou actuel dans `genererContenuQualite()` ne calcule ni ne plafonne ce volume par séance, pour aucune des zones I/T/V. Piste retenue mais non codée : plafonner `reps` (ou la durée totale du bloc pour les sous-types multi-blocs) selon `min(repère absolu, volumeHebdoCible × pourcentage de la zone)` converti en durée à l'allure de la zone concernée, en plus du plafond `cap` par niveau déjà existant — les deux contraintes coexisteraient (celle qui est la plus stricte prévaut, jamais une remplace l'autre).

**Rôle du niveau, clarifié le 06/08/2026** : `base`/`cap` par niveau et le plafond Daniels ne se recouvrent pas — `base`/`cap` pilote *la vitesse de progression et jusqu'où le moteur veut emmener le coureur selon son expérience déclarée* ; le plafond Daniels dit *ce que le volume hebdo réel autorise physiologiquement*, indépendamment du niveau. Le plafond Daniels est une formule universelle (mêmes pourcentages quel que soit le niveau) — le niveau influence le résultat uniquement de façon indirecte, via le volume hebdo réel généralement corrélé au niveau (un débutant a structurellement moins de volume qu'un confirmé). Cas limite non couvert : un débutant à volume élevé pour son niveau obtiendrait un plafond haut alors que sa capacité de récupération reste débutante — angle mort assumé du principe Daniels, pas résolu ici.

**Point d'attention pour l'implémentation** : le volume hebdo cible de la semaine (`volumeCibleKm`, déjà disponible dans `generatePlan()`) est la bonne référence à utiliser, pas le volume de départ déclaré — il évolue au fil du plan (progression, décharge, affûtage), et le plafond par séance doit suivre cette même évolution plutôt que rester figé sur une valeur de début de plan. Se recalcule automatiquement à chaque régénération du plan (génération initiale, tous les leviers de l'accordéon "Modifier mon plan" — Objectif/Jours/Volume/Date de course —, `appliquerAdaptations()`, `regenererStructuresIntervalles()`), puisque ces fonctions appellent toutes `genererContenuQualite()` avec le `volumeCibleKm` à jour de la semaine concernée — aucun code supplémentaire nécessaire ailleurs que dans cette fonction elle-même.
...
- Jack Daniels' Running Formula, chapitre 4, figure 4.1 (fichier projet) — plafond de volume I par séance : min(temps 10K, 8% volume hebdomadaire) (section 43)
