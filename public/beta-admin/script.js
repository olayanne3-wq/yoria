...
$("#cards-signalements").innerHTML=x.map(sg=>`<article class="candidate"><div style="min-width:0;"><strong>${esc(TS[sg.type]||sg.type)}</strong><div class="chips"><span class="badge ${esc(sg.statut)}">${esc(SS[sg.statut]||sg.statut)}</span></div><small style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:4px;">${esc(sg.message||"—")}</small><small style="color:var(--muted);">${esc(date(sg.created_at))}</small></div><select data-signalement-statut="${sg.id}" style="flex-shrink:0;"><option value="nouveau" ${sg.statut==="nouveau"?"selected":""}>Nouveau</option><option value="en_cours" ${sg.statut==="en_cours"?"selected":""}>En cours</option><option value="resolu" ${sg.statut==="resolu"?"selected":""}>Résolu</option></select></article>`).join("")||'<div class="empty">Aucun signalement.</div>';}
