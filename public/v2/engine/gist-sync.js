...
export function getGithubToken(storage = localStorage) {
...
export function setGithubToken(token, storage = localStorage) {
  storage.setItem('lk_github_token', token);
}
export function getV2GistId(storage = localStorage) {
  return storage.getItem('v2_gist_id') || '';
}
export function setV2GistId(id, storage = localStorage) {
  storage.setItem('v2_gist_id', id);
}

export async function chargerPlans(storage = localStorage) {
...
export async function chargerPlansOriginal(storage = localStorage) {
  const token = getGithubToken(storage);
  const gistId = getV2GistId(storage);
  if (!token || !gistId) return [];
  try {
...
// rien.
export async function ajouterPlanOriginal(plan, storage = localStorage) {
  const plansOriginalExistants = await chargerPlansOriginal(storage);
  if (plansOriginalExistants.some(p => p.id === plan.id)) return;
  const plansActif = await chargerPlans(storage);
  const nouveauxPlansOriginal = [...plansOriginalExistants, { ...plan, sauvegardeLe: new Date().toISOString() }];
  const token = getGithubToken(storage);
  if (!token) {
...
export async function ecrireListePlans(plans, storage = localStorage) {
  const token = getGithubToken(storage);
  if (!token) {
    throw new Error("Aucun token GitHub renseigné — impossible de sauvegarder.");
  }
  ...
  const body = {
    description: 'Yoria v2 — plans sauvegardés',
    public: false,
    files: { [GIST_FILENAME]: { content: JSON.stringify({ plans, plansOriginal }, null, 2) } }
  };
  const gistId = getV2GistId(storage);
  const url = gistId ? 'https://api.github.com/gists/' + gistId : 'https://api.github.com/gists';
  const method = gistId ? 'PATCH' : 'POST';
 ...
 * entre les deux.
 */
export function trouverPlanEnConflit(plans, dateDebut, dateFin, idAExclure) {
  return plans.find(p => {
...
export async function supprimerPlan(planId, storage = localStorage) {
  const plansExistants = await chargerPlans(storage);
  const plans = plansExistants.filter(p => p.id !== planId);
  return ecrireListePlans(plans, storage);
}

export async function renommerPlan(planId, nouveauNom, storage = localStorage) {
  const plansExistants = await chargerPlans(storage);
