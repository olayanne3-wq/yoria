export const handler = async (event) => {
  ...
  if (path === "/activities") {
    ...
    // Dates des séances intensives passées depuis le client (dynamique)
    const intervalDatesParam = event.queryStringParameters?.interval_dates;
    const INTERVAL_DATES = intervalDatesParam ? intervalDatesParam.split(",") : [];

    // Date de début du plan passée depuis le client
    const planStartParam = event.queryStringParameters?.plan_start;
